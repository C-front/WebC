...
#define STYLE() SCOPE(fputs("<style>", fp), fputs("</style>", fp))
...
#define SCRIPT_SRC(src) fprintf(fp, "<script src='%s'></script>", src)
#define SCRIPT(content) fprintf(fp, "<script>%s</script>", content)
...
#define BUTTON(attributes, content) fprintf(fp, "<button %s>%s</button>", attributes, content)
